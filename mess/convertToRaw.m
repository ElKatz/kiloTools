function [raw, rawInfo] = convertToRaw(dataFileType, opts)
...
if ~isfield(opts, 'performSegmentation'),   opts.performSegmentation = true;    end 
...
for iF = 1:nFiles
    ...
    %% appending data to files within data folder, under 'convertedToRaw' 
    % (only performed from spkData and spkTime:)
    if isfield(opts, 'saveDat') && opts.saveDat
        fprintf('\n\tAppending to file...')

        % spkData:
        spkDataPath = fullfile(dataFolder, 'convertedToRaw', spkDataFileName);
        fidout = fopen(spkDataPath, 'a'); % opening file for appending
        fwrite(fidout, rawSpk.samples, 'int16');
        fclose(fidout);

        % spkTime:
        spkTimePath = fullfile(dataFolder, 'convertedToRaw', spkTimeFileName);
        fidout = fopen(spkTimePath, 'a'); % opening file for appending
        fwrite(fidout, rawSpk.tSamples, 'double');
        fclose(fidout);
        fprintf('\n\tdone');
    end
    ...
    if nargout > 0 || (isfield(opts, 'saveMat') && opts.saveMat)
        ...
        % event time & value:
        tEvent = [tEvent(:); rawSpk.tEvent(:)];
        vEvent = [vEvent(:); rawSpk.vEvent(:)];
    end
